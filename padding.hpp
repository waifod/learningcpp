...
int main() {
  ...
  if (sizeof(struct foo3) == 16) {
    printf("This looks like a 64-bit machine.");
  } else if (sizeof(struct foo3) == 6) {
    printf("This looks like a 32-bit machine.");
  } else {
    printf("Huh? The word size of this machine is not obvious");
  }

  if ((offsetof(struct foo1, x) % sizeof(long)) == 0) {
    printf("Self-alignment seems to be required.");
  } else {
    printf("Self-alignment test of type long failed.");
  }
